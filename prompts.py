...
def get_combat_prompt(game_instance: GameState, scene_index: int, num_scenes: int):
...
The story is on scence {scene_index + 1} of {num_scenes}.
...
For the battle summary, briefly capture the context.
...
def get_action(combat_scene: CombatScene, player: Player, player_response: str, event: str):
    return f"""
...
Please select the single most applicable attribute required for the requested response: ['STRENGTH', 'INTELLIGENCE', 'CHARISMA']

The difficulty class (DC), should take into account the player's stats. 

If the response is logical and aligns with their stats then the DC should be low (5-10). 

If it is a risky action that could be possible with moderate likelihood then DC should be between 10 and 15.

For very unlikely but feasible responses it should be between 15 and 20. 

Finally, for nonsensical requests that do not match the narrative, you can make the DC 50 to indicate an impossible response.

"""

def get_action_result_prompt(combat_scene: CombatScene, actor: str, requested_action: str, action_result: bool, impact: str):
    return f"""
You are a dungeon master for a D&D game and must a narrative outcome for the following dice roll in third person using names (≤15 words).
...
In the outcome, explicitly include the action that was attempted as well as the result.
"""  
...
def get_next_event_prompt(combat_scene: CombatScene, next_player:str):
    return f"""
Current scene summary:
{combat_scene}

Please include a new event that '{next_player}' has to respond to in third person using their name (≤20 words).
"""
...
Based on the summary below, please determine whether the scene is complete.
...
def update_game_summary_prompt(old_summary : str, scene_summary : str):
    return f"""
Please update the following game summary:
{old_summary}

With a new scene that has transpired:
{scene_summary}

Please keep the summary less than 80 words while minimizing information loss.
"""